export class Monster {
  constructor(path, monsterImages, level, monsterUnlock) {
    // 생성자 안에서 몬스터의 속성을 정의한다고 생각하시면 됩니다!
    if (!path || path.length <= 0) {
      throw new Error("몬스터가 이동할 경로가 필요합니다.");
    }

    const allowedMonster = this.getAllowedMonster(monsterUnlock, level).monster_id

    this.monsterNumber = Math.floor(Math.random() * (allowedMonster.length - 1) + 1); // 몬스터 번호 (현재 스테이지에 나올 수 있는 몬스터중 소환)
  ...
  // stage에 따라 몬스터 ID 배열을 가져오는 메서드
  getAllowedMonster(monsterUnlock, level) {
    return monsterUnlock.find((unlock) => unlock.wave_id - 999 === level)
  }
